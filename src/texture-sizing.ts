...
import { resolveUiTexturePixelRatio } from "./graphics-policy";
 ...
 */
export function resolveCurrentUiTexturePixelRatio(): number {
  return resolveUiTexturePixelRatio();
}

/** Resolves logical dimensions + pixel ratio into physical texture pixels. */
export function resolveTexturePixelSize(input: TexturePixelSizeInput): TexturePixelSize {
  const rounding = input.rounding ?? "integer";
  const maxTextureSize = sanitizeMaxTextureSize(input.maxTextureSize);
  return {
    width: Math.min(
      roundPixelExtent(input.logicalWidth * input.pixelRatio, rounding),
      maxTextureSize,
    ),
    height: Math.min(
      roundPixelExtent(input.logicalHeight * input.pixelRatio, rounding),
      maxTextureSize,
    ),
  };
}
...
export function textureCanvasScaleFor(
  logicalWidth: number,
  logicalHeight: number,
  pixelSize: TexturePixelSize,
): TextureCanvasScale {
  return {
    x: pixelSize.width / Math.max(1, logicalWidth),
    y: pixelSize.height / Math.max(1, logicalHeight),
  };
}

function sanitizeMaxTextureSize(maxTextureSize: number | undefined): number {
  if (maxTextureSize === undefined || !Number.isFinite(maxTextureSize)) {
    return Number.MAX_SAFE_INTEGER;
  }
  return Math.max(1, Math.floor(maxTextureSize));
}

function roundPixelExtent(value: number, rounding: TexturePixelRounding): number {
  const integer = Math.max(1, Math.ceil(Number.isFinite(value) ? value : 1));
  if (rounding === "power-of-two") {
    return nextPowerOfTwo(integer);
  }
  if (rounding === "even" && integer % 2 !== 0) {
    return integer + 1;
  }
  return integer;
}
